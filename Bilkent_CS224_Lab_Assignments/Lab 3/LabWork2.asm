...
## Lab Work 2 Part 4
## A program that creates a linked list, prints that
## and creates a copy of it iteratively.
#################################
#					 	#
#		text segment		#
#						#
...
# when we print linked list we can differentiate the node content from the node sequence no (1, 2, ...).
	...
	move 	$s7, $a0
	add	$s0, $0, 10	# $s0: no. of nodes to be created.
	li	$s1, 1		# $s1: Node counter
# Create the first node: header.
# Each node is 8 bytes: link field then data field.
	li	$a0, 8
	...
	lw	$s6, 0($s7)	# $s1: Address of  next node
	lw	$s5, 4($s7)
# OK now we have the list head. Save list head pointer 
	move	$s2, $v0	# $s2 points to the first and last node of the linked list.
	move	$s3, $v0	# $s3 now points to the list head.
	sw	$s5, 4($s2)	# Store the data value.
...
space: .asciiz " "
enter: .asciiz "\n"
revList: .asciiz "Reversed linked list:\n"
line:	.asciiz "\n --------------------------------------"
